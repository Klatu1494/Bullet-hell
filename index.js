addEventListener("load", () => {
  ...
  class Game {
    ...
    init() {
      ...
      this.attDelay = 0;
      this.mousePosition = new Vector(WIDTH / 2, HEIGHT / 2);
  ...
  class Skill {
    ...
    levelUp() {
      ...
      this.level++;
      this.updateLevelSpan();
      if (this.level === 10) this.row.remove();
  ...
  class Proyectile extends RoundObject {
    ...
    move() {
      ...
      if (this.position.x < this.radius) {
        ...
        this.speed.x *= -1;
        this.addBounce();
      } else if (WIDTH - this.radius < this.position.x) {
      ...
      if (this.position.y < this.radius) {
        this.delete();
      } else if (HEIGHT - this.radius < this.position.y) {
        this.delete();
      }
      this.div.style.left = (this.position.x - this.radius) + "px";
      this.div.style.top = (this.position.y - this.radius) + "px";
  ...
  class Enemy extends RoundObject {
    constructor(life, radius, speed, attackDelay, dropChance) {
      super(new Vector(radius + Math.random() * (WIDTH - 2 * radius), -radius), radius, new Vector(speed, speed));
    ...
    move() {
      ...
      this.div.style.left = (this.position.x - this.radius) + "px";
      this.div.style.top = (this.position.y - this.radius) + "px";
      if (HEIGHT + this.radius < this.position.y) {
        this.die();
  ...
  function loop() {
    ...
    gameDiv.style.backgroundPosition = "0 " + game.backgroundPosition + "px";
    player.speed = new Vector(0, -1);
    if (pressedKeys[player.up]) player.speed.y -= 4;
    if (pressedKeys[player.down]) player.speed.y += 4;
    ...
    if (!game.enemies.size && game.currentLevel.monsterClasses.length <= game.currentLevel.index && 0 < player.life) {
      ...
      for (var proyectile of game.proyectiles) proyectile.delete();
      player.life = player.maxLife;
      game.currentLevel.maxStarsReached = Math.max(game.currentLevel.maxStarsReached, 1 + game.damageStar + game.bottomStar);
  ...
  function save() {
    localStorage.savedStats = JSON.stringify({
      skillPoints: skillPoints,
      stars: levels.map(a => a.maxStartReached),
      skillLevels: skills.map(a => a.level)
    });
  }
  ...
  var skillPoints;
  var levelReached;
  var player = new Player(new Vector(WIDTH / 2, HEIGHT / 2), 87, 65, 83, 68, 16);
  var skills = [
    new Skill(player, () => {
      player.maxLife += 10;
      player.life += 10;
  ...
  (function() {
    ...
    var savedStats = JSON.parse(localStorage.savedStats || '{"skillPoints":0,"stars":[0],"skillLevels":[]}');
    for (var i = 0; i < savedStats.skillLevels.length; i++) {
      skillLevel = savedStats.skillLevels[i];
      for (var j = 1; j < savedStats.skillLevels[skillLevel]; j++) {
        skills[i].levelUp();
      }
    }
    var level = 1;
    ...
    levelSelectionDiv.style.width = WIDTH + "px";
    levelSelectionDiv.style.height = HEIGHT + "px";
    for (i = 0; i < 30; i++) monsters.push(Bug);
    pushLevel();
    for (i = 0; i < 16; i++) {
      monsters.push(Shooter);
      for (j = 0; j < 3; j++) monsters.push(Bug);
    }
    pushLevel();
    ...
    for (i = 0; i < 10; i++) {
      ...
      div.style.left = 290 + 85 * (i % 2) + "px"
      div.style.top = 400 - 45 * i + "px"
      div.addEventListener("click", function() {
        if (this <= levelReached) levels[this].start();
      }.bind(i));
      divs.push(div);
    }
    skillPoints = savedStats.skillPoints;
    levelReached = levels.length;
    for (i = 0; i < levelReached; i++) {
      divs[i].classList.remove("inactive");
      if (!savedStats.stars[i]) levelReached = i;
    }
  })();
  ...
  addEventListener("keydown", e => {
    ...
    if (e.keyCode === ESC) goToLevelSelection();
  });
  document.getElementById("skill-points").innerText = skillPoints;
  goToLevelSelection();
});